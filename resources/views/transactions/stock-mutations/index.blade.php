...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-white tracking-wide">Mutasi Barang</h1>
            <p class="text-xs text-slate-400">Perpindahan stok antar lokasi/gudang.</p>
        </div>
        
        @can('mutations.create')
            <a href="{{ route('stock-mutations.create') }}" class="rounded-lg bg-indigo-600 px-4 py-2.5 text-xs font-semibold text-white hover:bg-indigo-500 shadow-lg shadow-indigo-600/20 transition-all flex items-center gap-1.5">
    ...
    <form method="GET" class="grid gap-5 rounded-xl border border-[#1e293b] bg-[#121826]/60 p-5 shadow-lg card-glow md:grid-cols-5">
        <div class="flex flex-col">
            ...
            <input id="q" name="q" value="{{ request('q') }}" placeholder="Cari nomor..." class="input-professional w-full">
        </div>
        ...
        <div class="flex items-end gap-2.5">
            ...
            <a href="{{ route('stock-mutations.index') }}" class="rounded-lg bg-[#090d16] border border-[#1e293b] px-4 py-2.5 text-xs font-semibold text-slate-300 hover:bg-slate-900 transition-all">Reset</a>
        </div>
    </form>
        ...
        <table class="min-w-full divide-y divide-[#1e293b] text-sm">
            <thead class="bg-slate-950/60 text-left text-xs uppercase text-slate-400">
                <tr>
                    ...
                    <th class="px-5 py-3.5 font-bold border-b border-[#1e293b]">Tujuan</th>
                    <th class="px-5 py-3.5 font-bold border-b border-[#1e293b]">Requested By</th>
            ...
            <tbody class="divide-y divide-[#1e293b]/40">
                @forelse($mutations as $mutation)
                    <tr class="hover:bg-slate-900/30 transition-colors">
                        <td class="px-5 py-4 font-mono text-xs text-white font-semibold">{{ $mutation->mutation_no }}</td>
                        <td class="px-5 py-4 text-slate-300">{{ $mutation->mutation_date?->format('d/m/Y') }}</td>
                        <td class="px-5 py-4 text-slate-300">{{ $mutation->sourceLocation?->name }}</td>
                        <td class="px-5 py-4 text-slate-300">{{ $mutation->destinationLocation?->name }}</td>
                        <td class="px-5 py-4 text-slate-300">{{ $mutation->requester?->name ?? '-' }}</td>
                        <td class="px-5 py-4">
                            @php
                                $statusStr = is_object($mutation->status) ? $mutation->status->value : $mutation->status;
                            @endphp
                        ...
                        <td class="px-5 py-4 text-right">
                            <a href="{{ route('stock-mutations.show',$mutation) }}" class="text-indigo-400 hover:text-indigo-300 font-semibold transition-colors">Detail</a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="7" class="px-5 py-8 text-center text-slate-500">Belum ada transaksi mutasi.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
